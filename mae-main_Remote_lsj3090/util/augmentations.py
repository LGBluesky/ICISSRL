...
class Augmentations:
    ...
    @staticmethod
    def RandomCrop(p: float=1., size: tuple=(224, 224)):
    ...
    @staticmethod
    def CenterCrop(size: tuple=(224, 224)):
    ...
    @staticmethod
    def Resize(size: tuple=(224, 224)):
        def resize(img: np.ndarray):
            w, h, ch = img.shape
            img = SpatialTrans.resize(img, size)
            return img
        return resize
...
class StylisticTrans:
    ...
    def sharp(img: np.ndarray, sigma: float):
        ...
        :return: a ndarray
        """
        kernel = np.array([[-1, -1, -1], [-1, sigma, -1], [-1, -1, -1]], float) / (sigma - 8)  # 锐化
    ...
    def normalization(img: np.ndarray, rng: list):
        ...
        lb, ub = rng
        delta = ub - lb
        return (img.copy().astype(float)/255.) * delta + lb
...
class SpatialTrans:
    ...
    def rotate(img: np.ndarray, angle: int):
        ...
        mat = cv2.getRotationMatrix2D((int(img.shape[0]//2), int(img.shape[1]//2)), angle, scale=1)
